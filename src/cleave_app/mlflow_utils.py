...
def log_cnn_training_run(config, 
                         model: Model, 
                         history: Any, 
                         dataset_path: str,
    ...
    with mlflow.start_run():
        ...
        mlflow.log_input(mlflow.data.from_pandas(df, source=dataset_path,
...
def log_mlp_training_run(config, 
                         model: Model, 
                         history: Any,
                         dataset_path: str, 
                         artifacts=Optional[Dict[str, Optional[str]]] = None) -> None:
...
def log_image_training_run(config, 
                         model: Model, 
                         history: Any, 
                         dataset_path: str,
                         artifacts=Optional[Dict[str, Optional[str]]] = None) -> None:
...
def log_xgb_training_run(config, 
                         model: Model, 
                         X_train: Any,
                         y_train: Any, 
                         dataset_path: str,
...
def log_cnn_test_results(tester,
                         run_name: str,
                         model_path: str,
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        df = pd.DataFrame({
            ...
            "predictions": [round(float(p[0,0]), 4) if tester.classification_type == "binary" else round(float(np.max(p[0])), 4) for p in predictions]
        })
