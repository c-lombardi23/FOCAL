...
def test_test_mlp(mocker, test_mlp_config):
    ...
    mock_tension_predictor = mocker.patch("cleave_app.commands.test_commands.TensionPredictor")
    mock_tester = mock_tension_predictor.return_value

    mock_tester.predict.return_value = (
        np.array([112., 120, 145]),
    ...
    mock_log_run = mocker.patch("cleave_app.commands.test_commands.log_regressor_test_results")
