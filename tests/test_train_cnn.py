...
def run_training_command(mocker, config, command_class, image_only=True):
    ...
    mock_train_ds = tf.data.Dataset.from_tensor_slices(((mock_images, mock_features), mock_labels)).batch(config.batch_size)
    mock_test_ds = tf.data.Dataset.from_tensor_slices(((mock_images, mock_features), mock_labels)).batch(config.batch_size)
    mock_class_weights = {0: 1.0}

    data_collector = mocker.patch("cleave_app.commands.train_commands.DataCollector")
    ...
    if image_only:
        mock_train_ds = tf.data.Dataset.from_tensor_slices((mock_images, mock_labels)).batch(config.batch_size)
        mock_test_ds = tf.data.Dataset.from_tensor_slices((mock_images, mock_labels)).batch(config.batch_size)
        data_collector_instance.image_only_dataset.side_effect = [mock_train_ds, mock_test_ds]
    ...
    mock_custom_model = mocker.patch("cleave_app.commands.train_commands.CustomModel")
    mock_custom_model_instance = mock_custom_model.return_value
    if image_only:
        mock_custom_model_instance.compile_image_only_model.return_value = mock_custom_model_instance.compile_model
    else:
    ...
    if image_only:
        mock_log_run = mocker.patch("cleave_app.commands.train_commands.log_image_training_run")
    else:
        mock_log_run = mocker.patch("cleave_app.commands.train_commands.log_cnn_training_run")
...
def test_train_cnn(mocker, train_cnn_config):
    ...
    run_training_command(mocker, train_cnn_config, command_class, image_only=False)

def test_train_image_only(mocker, train_cnn_config):
    command_class = TrainImageOnly
    run_training_command(mocker, train_cnn_config, command_class, image_only=True)
